...
class _Product {
	...
	public function details($id) {
		$this->db->query('SELECT `p`.*, `s_currency`, (SELECT `currency_symbol` FROM `sys_country` WHERE `currency` = `s_currency`) `currency_symbol` FROM `product` `p` JOIN `seller` `s` ON (`p`.`p_sellerstamp` = `s`.`id`) WHERE `p`.`id` = :id');
